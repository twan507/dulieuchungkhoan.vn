...
def _danh_muc(conn: sa.Connection, keyword: str | None) -> list[dict]:
    return [dict(r._mapping) for r in conn.execute(_SQL_DANH_MUC, {"kw": keyword})]
...
def chuoi_vi_mo(conn: sa.Connection, code: str | None = None, keyword: str | None = None,
    ...
    if loai_rows:
        ...
        return to_json(co_du_lieu(du_lieu, ma=code, ten=tai_san.name_vi, don_vi=tai_san.unit,
                                  loai_gia=loai_gia, cac_loai_gia_co_san=cac_loai, da_cat=da_cat))

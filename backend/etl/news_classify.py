"""Lưới AI phân loại tin (lát 9a, spec 2026-09-06-news-classify-llm): taxonomy 3 nhóm / 21 sub / x (news-pipeline §3, §7),
...
def run(limit: int | None = None, per_group: int | None = None, thinking: str = "adaptive", dry_run: bool = False, out: str | None = None,
    ...
    if dry_run:
        ...
        try:
            ...
            return 1  # no-run: dry-run, run_id=None — không có sổ để đóng
        except LLMError as e:
            log.error("%s", e)
            return 2

...
**Từ 2026-09-04 task chạy `Interactive`** (quyết định chủ dự án — S4U đòi cửa sổ admin mỗi lần thêm task): đăng ký bằng `pwsh scripts/register-tasks.ps1` **không cần admin**; mỗi job hiện một cửa sổ `cmd` có **tiêu đề = tên task** và **một dòng mô tả ngắn** khai trong script (`-Description`: đây là task gì, chạy bao lâu, kết thúc thế nào; tiếng Việt có dấu nhờ `chcp 65001`) — không đường dẫn, không lệnh (output thật của job vào file log). Job giá in thêm dòng `bắt đầu HH:MM · con trỏ … · còn N mã · hạn …` lúc mở và dòng `XONG HH:MM · …` giữ 20 giây lúc đóng, ghi thẳng ra console. **Nút X của cửa sổ bị chính job khoá** lúc khởi động ([`core/console.py`](core/console.py), chỉ khi wrapper đặt `DLCK_LOCK_CONSOLE=1` — chạy tay từ terminal không bị) vì console không thể hỏi lại xác nhận; dừng có chủ đích bằng **Ctrl+C** trong cửa sổ (job `price` ghi `ops.etl_run` = `failed: dừng tay (Ctrl+C)`, thoát mã 130, con trỏ backfill giữ nguyên, và **giữ cửa sổ 20 giây** với một dòng "ĐÃ DỪNG … con trỏ X đã lưu" ghi thẳng ra console — output thường của job đã đi vào log nên cửa sổ vốn không hiện gì; guard từ chối và lỗi cũng có lời chào cuối như thế) hoặc `Stop-ScheduledTask <tên>` (giết tiến trình — lượt treo `running`, đóng tay nếu cần). Task khởi động qua **`conhost.exe`** để mở console cổ điển — trên Windows 11 terminal mặc định là Windows Terminal, cửa sổ thật thuộc `WindowsTerminal.exe` và khoá X không tới được (đo 2026-09-04). ⚠️ Console cổ điển có QuickEdit: **bấm chuột vào trong cửa sổ** là vào chế độ Select (tiêu đề `Select …`) và **job bị treo** tới khi bấm Esc — chỉ nhìn, đừng bấm vào. ⚠️ Task S4U cũ chỉ xoá được bằng admin — sau lần dọn 2026-09-04 không còn task S4U nào.

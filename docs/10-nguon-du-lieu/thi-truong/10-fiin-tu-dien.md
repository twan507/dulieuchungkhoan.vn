...
Bảng đầy đủ `code → name → unit`: [Phụ lục A](phu-luc-A-ma-field.md#a3-bảng-tra-83-tiêu-chí-từ-getscreenerparameters).
...
- Bảng này **không phủ** các mã chỉ tiêu báo cáo tài chính chi tiết (`bsa*`, `isa*`, `isb*`, `cfa*`, `nob*`). Chúng được giải mã bằng nguồn khác — xem [Phụ lục A §A.5](phu-luc-A-ma-field.md), 729 mã lấy từ bundle JS của ứng dụng FiinTrade, độ phủ 100%.
...
Danh sách đầy đủ và lý do từng nhóm: [ADR 0002 — Chọn nguồn dữ liệu](../../00-tong-quan/quyet-dinh/0002-chon-nguon-du-lieu.md).

...
| Việc | Ghi chú |
|---|---|
| **Đo tỷ lệ dedupe thật** | ✅ đo + đã dựng khoá thứ tư (`merged_near`, pg_trgm 0,6 — §9.4) 2026-09-06 (`--loop` lát 8, 91 vòng · 152.569 item · 1.860 mới): gộp theo **tiêu đề y hệt chỉ ≈ 0,5 % bài** (14 gộp tiêu đề, 0 gộp URL) — ước "3,5 lần" của §9.2 quá xa thực tế; tin "cùng chuyện, khác tít" chỉ đo được bằng embedding (lát 9b). Ngân sách phân loại vì thế tính trên **toàn bộ bài mới** (~170–315/ngày thường), không chia 3,5 **Đo lại 2026-09-07 trên 25 giờ liên tục có trọn ngày làm việc** (301 vòng, AC7 lát 8): gộp **6/476 = 1,26 %** (`merged_url` **0**, `merged_title` 6), 470 bài mới — cùng hướng với 0,5 % của lát 9a, xa hẳn ước 3,5 lần của §9.2. ⚠️ Vòng lặp đo được chạy **code trước lát 9b**: `stats` không có `merged_near`, tức khoá `pg_trgm` **chưa chạy vòng thực tế nào** — số trên là của **ba khoá**. **Vòng lặp đã khởi động lại 2026-09-07 13:21 bằng code trên `main`** (chủ dự án chốt) ⇒ khoá thứ tư từ đây mới thực sự chạy; số dedupe **bốn khoá** của vòng chạy thật đọc được từ 13:30 ngày 08/09. **Phát lại offline trên chính kho đó (2026-09-07): khoá thứ tư gộp thêm 22/485 = 4,5 %**, tổng bốn khoá ≈ **5,7 %** — gấp ~4,5 lần ba khoá ([ledger lát 8 §7](../90-records/plans/2026-09-05-news-collect/ledger.md)). Chi tiết: [ledger lát 8 §7](../90-records/plans/2026-09-05-news-collect/ledger.md). |
...
3. **Chốt mô hình embedding** — ⏸️ **hoãn có chủ đích, không chặn ai** *(rà 2026-09-07)*: lát 9b đã thay embedding bằng `pg_trgm` cho dedupe, lát 10 dựng tầng ngữ nghĩa mà **không** cần tìm kiếm khái niệm. Lý do và ba phương án ở **§12** (chủ sở hữu duy nhất của mục này) — mở lại khi thật sự dựng tìm kiếm ngữ nghĩa trên kho tin. *(`embo-01` của MiniMax bị chặn dưới Token Plan và 1536 chiều ≠ `halfvec(768)`.)*

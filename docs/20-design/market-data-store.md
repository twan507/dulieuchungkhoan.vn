...
**Nhịp thật — cài đặt 2026-09-04 (lát 4), bỏ trần ngày 2026-09-09 (lát 13).** Quét sàn không chạy thành đợt lớn mà **cuốn chiếu theo nhịp**: mỗi lượt lấy **mọi** cặp `(issuer, kind)` đã tới nhịp, nhóm có `checked_at` cũ nhất trước, chưa kiểm bao giờ thì lên đầu.

| Kind | Nhịp | Phủ trọn sàn sau |
|---|---|---|
| `snapshot` | 90 ngày | 90 ngày |
| `ownership` · `valuation` · `dividend` | 30 ngày | 30 ngày |

🔴 **Trần lời gọi mỗi ngày (24 + 70 + 70 + 70 = 234) đã BỎ ngày 2026-09-09** *(quyết định chủ dự án, lát 13)*. Trần đó vốn để rải tải khi chưa có lịch tự động; nay scheduler chạy đều mỗi ngày nên chính **nhịp** đã rải hộ: ở trạng thái ổn định mỗi ngày chỉ có ~1/30 (hoặc ~1/90) số cặp tới hạn, tức vẫn cỡ **234 lời gọi/ngày**. Khác biệt nằm ở hai ca biên: kho mới hoặc sau một quãng không chạy thì lượt kế **quét trọn phần tồn** thay vì cắt và kéo dài nhiều ngày — đúng thứ ta muốn khi đang nạp đầy kho. Trần còn lại là `--max-minutes` (dừng sau target đang dở) và trần 300 issuer của nhánh trigger; không có đỉnh tải cố định và không cần mốc lịch thứ hai. Sổ kiểm `ops.snapshot_check` *(migration `0016`, một dòng mỗi `(issuer, kind)` — tối đa 6.092 dòng = 1.523 × 4 kind, không phình; 292 dòng sau ngày quét sàn đầu tiên 2026-09-04)* làm hai việc: cấp danh sách tới hạn, và **đếm lỗ của lịch** — mỗi lần quét sàn tìm ra thay đổi mà trigger không bắn là một lỗ, đếm được ở `ops.etl_run.stats`. `checked_at` **chính là con trỏ**, nên job không cần con trỏ riêng.
...
> **Hệ quả cho ETL:** nạp `metric_dictionary` từ **hai nguồn**, không phải một. `GetScreenerParameters` cấp 83 chỉ tiêu thị trường kèm `valueRange`; file [`field-dictionary.json`](../../backend/etl/data/field-dictionary.json) cấp 729 chỉ tiêu (556 BCTC + 173 tỷ số) kèm **`don_vi_du_lieu`** cho 727 mã (99,7%), **392 mã đã xác thực** bằng đẳng thức kế toán và kiểm nhất quán thang. **Đưa cả bảy phép kiểm đó vào bộ giám sát hợp đồng** — chúng phát hiện được cả việc nguồn đổi thang đơn vị lẫn việc dữ liệu hỏng. File JSON là ảnh chụp tĩnh: bundle đổi hash mỗi lần FiinTrade deploy, nên đặt việc trích lại vào [bộ giám sát hợp đồng](#71-giám-sát-hợp-đồng-dữ-liệu) thay vì coi nó là bất biến.
...
>
> **2026-09-05 tối — lát 7b, cập nhật trong phiên:** ba job `yahoo`/`binance`/`wichart` thêm cờ `--intraday`, chạy lặp trong giờ giao dịch (Yahoo mỗi 10 phút, Binance/WiChart mỗi 5 phút). Hệ quả: dòng mới nhất của mỗi mã trong `asset.ohlc_daily` (Yahoo 54 mã, Binance 11 mã) và điểm ngày hiện tại của series tần suất ngày WiChart trong `asset.price_daily`/`macro.observation` **có thể đang chạy** — bị ghi đè liên tục tới khi phiên đóng. **Không có cột cờ** đánh dấu "đang chạy"; tầng đọc phải tự suy từ `calendar`/lịch sàn, không đọc thẳng dòng cuối như đã đóng. Tên gọi "intraday" chỉ mô tả **tần suất cập nhật**, KHÔNG phải dữ liệu nến trong ngày — mỗi mã vẫn một dòng một ngày. Thêm 17 asset Yahoo FX `fx.usd_<ccy>.market` (mốc nửa đêm London, tách khỏi 6 cặp fixing ECB); `fx.usd_cny` nay thuần ECB (chuỗi từ 2000-01-13), FRED `DEXCHUS` không nạp nữa. 🔴 `close` lịch sử của `fx.usd_<ccy>.market` thực ra là giá **đầu ngày** London, không phải giá cuối ngày (Bẫy 4, [`yahoo.md`](../10-sources/global/yahoo.md) §5.5, đo 2026-09-05 tối) — **tầng đọc:** biểu đồ lịch sử FX lấy `fx.usd_<ccy>` (ECB), `.market` chỉ dùng điểm ngày hiện tại và high/low. Hồ sơ: [lát 7b](../90-records/plans/2026-09-05-intraday-refresh/).
>
> **2026-09-06 — lát 8:** miền `news` có job `news.collect`/`news.backfill_sitemap`; `data_domain_state ('news', <báo>)` 8 báo; bằng chứng thô chỉ danh sách + bài từ chối. **Lát 8b (cùng ngày):** backfill sitemap thành job theo nguồn `news.backfill_sitemap:<source>` (`tinnhanhck` · `bnews` · `nguoiquansat`), con trỏ riêng từng nguồn trong `stats.cursor`; tên cũ `news.backfill_sitemap` chỉ còn ở các lượt lịch sử của TinnhanhCK (code vẫn đọc làm fallback).

...
| **`etl`** | **Scheduler + job theo lịch (batch)** | ❌ không *(chỉ metrics)* | Bảng tham chiếu · giá EOD · snapshot · screener · sự kiện · BCTC (Postgres) · crawl OMO/vĩ mô/quốc tế · tin | Tiến trình dài **là scheduler** (`python -m etl` không tham số, lát 13): mỗi 20 s đọc `ops.etl_run`, spawn `python -m etl <job>` làm tiến trình con, chạy rồi thoát; một số job kích hoạt theo sự kiện (`snapshot` sau `events`), một job là daemon con (`news --loop`) | `api`, `ingester` |
...
4. **`api` không có trạng thái cục bộ.** Mọi trạng thái chia sẻ (session realtime, current-state) nằm ở Redis/DB, để nhân bản ngang không lệch nhau.
    ...
    **Scheduler cũng không giữ trạng thái bền.** Nguồn sự thật về "mốc nào đã chạy" là bảng `ops.etl_run`, không phải bộ nhớ tiến trình: khởi động lại giữa ngày thì nó đọc sổ, thấy mốc nào chưa `success` thì chạy bù. Thứ duy nhất nằm trong RAM là nhịp `--intraday`, backoff của daemon và **hạ nhiệt 10 phút sau khi một con thoát mã ≠ 0** — mất cả ba khi khởi động lại đều vô hại. Chi tiết vận hành (bảng lịch, sáu luật chạy bù, khoá, log, mã thoát): [`backend/README.md`](../../backend/README.md) mục Scheduler.
5. ✅ **Lát 13 (2026-09-09): lịch nằm trong chính service `etl`.** `python -m etl` không tham số = scheduler; bảng lịch là dữ liệu trong code (`backend/etl/scheduler/schedule.py`), không phải cấu hình ngoài. Chống chạy chồng **hai lớp**: khoá advisory Postgres trong `open_run` (lớp trong, đúng cho cả lượt chạy tay lẫn container khác) và dict tiến trình con theo tên job của runner (lớp ngoài). Log mỗi job một file theo ngày trong volume `etl_logs`. Cùng một code chạy native trên dev (`uv run python -m etl`) và trong container trên VPS.

6. ✅ **Lát 12 (2026-09-08): cơ chế Task Scheduler về hưu trong code.** `register-tasks.ps1` và `core/console.py` đã xoá; mọi tiến trình chạy bằng `docker compose` ở gốc repo, `ingester` là daemon tự ngủ ngoài phiên; `docker stop` gửi `SIGTERM`: job `etl` đóng sổ `ops.etl_run` và thoát 130, ingester đặt `stop` và đóng phiên đúng đường deadline (xả + đối chứng), thoát 0/1. 11 task Windows về hưu — gỡ bằng một lệnh PowerShell ở Task 12 (chủ dự án). Sau reboot, `restart: unless-stopped` đưa `api`/`etl`/`ingester` lên nhưng `depends_on` KHÔNG được đánh giá lại: ingester có thể trả 3 vài lượt tới khi kho healthy rồi tự ổn — ồn log, không mất dữ liệu (review lát 12). Phần dưới giữ làm lịch sử.

    ✅ **Task Scheduler đã chuyển `S4U` 2026-08-28 — cửa sổ `cmd` không còn.** 🔁 **Đảo lại `Interactive` 2026-09-04 — quyết định chủ dự án, xem cuối mục.** **Cả 11 task** mang `LogonType=S4U` — 7 task đầu (`dlck-ingester`, `dlck-ingester-measure`, `dlck-refdata`, 4 task OMO) từ 2026-08-28, `dlck-screener` 15:20 và `dlck-events` **18:10** từ **2026-09-03**, `dlck-price` **15:40** và `dlck-price-backfill` **thứ 7 00:05** (giới hạn chạy `PT72H`) từ **2026-09-04** *(AC8 [ledger lát 3](../90-records/plans/2026-09-03-price-daily-etl/ledger.md) — kèm bài học: `DaysOfWeek` là bit cờ số, `ExecutionTimeLimit` được chuẩn hoá thành `PT72H`)* *(giờ của `dlck-events` chọn 18:00 lúc đầu, dời sang 18:10 ngay trong ngày vì đụng `dlck-omo-1800` — cùng lý do `dlck-screener` đặt lệch 15:30)*, `RunLevel=Limited`, `UserId=tuanb` *(nghiệm thu bằng cách soi `Principal` TỪNG task, không soi trạng thái — `Assert-TaskPrincipal` trong `scripts/register-tasks.ps1`, soi CẢ `LogonType` LẪN danh tính chạy dưới quyền ai)*. Trước đó là `Interactive` với ba hệ quả: (a) chỉ chạy khi có người đăng nhập; (b) mỗi lượt hiện một cửa sổ `cmd` — bấm nhầm dấu X trên cửa sổ `dlck-ingester` là **giết phiên ghi tick**, thứ duy nhất không crawl lại được; (c) không có quyền admin. **(b) là cái đắt nhất và nay đã hết.**

    🔁 **Đảo lại `Interactive` 2026-09-04 (quyết định chủ dự án).** S4U đòi cửa sổ Run as Administrator **mỗi lần thêm task** (lát 1, 2, 3 đều phải xin admin), và xoá task S4U cũng cần admin — mệt và khó quản trên máy dev, trong khi chủ dự án **thích thấy cửa sổ** để biết task nào đang chạy. Từ đây script mặc định `Interactive`, không cần admin; cửa sổ `cmd` có **tiêu đề = tên task** và in một dòng `[dlck-price] python -m etl price -- log: …` rồi chạy (output thật vẫn vào file log). Ba hệ quả cũ quay lại, chấp nhận có chủ đích: (a) chỉ chạy khi có người đăng nhập — vốn không mất gì vì Docker Desktop cũng nằm trong session; (b) **bấm nhầm X là giết job** — đau nhất với `dlck-ingester` (tick không lấy lại được); giảm bằng code: job **khoá nút X của chính cửa sổ** lúc khởi động (`core/console.py` xoá `SC_CLOSE` khỏi system menu — X mờ, Alt+F4 và "Close window" cũng bị chặn; console không thể hỏi xác nhận vì Windows giết tiến trình sau `CTRL_CLOSE_EVENT`), chỉ khi wrapper đặt `DLCK_LOCK_CONSOLE=1` để terminal chạy tay không mất nút X; dừng có chủ đích bằng Ctrl+C hoặc `Stop-ScheduledTask`; (c) không có quyền admin — không job nào cần. Mô hình đích vẫn là **scheduler trong container `etl`** — nay là **lát 13** trong chuỗi chuẩn của [roadmap](../00-overview/roadmap.md) — bước chuẩn hoá cách chạy sau cùng, còn "lên VPS" là **lát 15** *(số cũ là 14; đẩy xuống 2026-09-07 vì chèn **lát 12 "chạy được trong container"** trước scheduler)*, sau cùng, sau vài ngày chạy thử trên dev *(đánh số lại 2026-09-04 tối; tên cũ "lát 7"/"lát 8")*; lúc đó Task Scheduler về hưu hẳn.
...
**Trong container** (lát 12, 2026-09-08; `etl-logs` thêm ở lát 13): `/app/backend` (WORKDIR) · `/app/database` · `/var/lib/dlck/{logs,measure,spill,etl-logs}` (**4 volume** — ba volume đầu của `ingester`, `etl_logs` của scheduler, biến `ETL_LOG_DIR`) · `/backups` (bind `CLICKHOUSE_BACKUP_DIR`) — **không có `/app/docs`**, code không đọc `docs/`, dữ liệu tra cứu ở `backend/etl/data/` (Task 10a). Compose: `docker-compose.yml` gốc + `docker-compose.vps.yml` qua `COMPOSE_FILE`.
...
**Cách chạy hồ sơ VPS:** `.env` đặt `COMPOSE_FILE=docker-compose.yml:docker-compose.vps.yml`, rồi

```bash
docker compose up -d --build
```

Overlay [`docker-compose.vps.yml`](../../docker-compose.vps.yml) + [`clickhouse/memory-vps.xml`](../../deploy/infra/clickhouse/memory-vps.xml). Không sửa file gốc: máy dev 64 GB ép xuống hồ sơ VPS chỉ làm test chậm mà không lộ thêm điều gì.

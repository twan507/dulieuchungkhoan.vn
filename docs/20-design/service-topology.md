...
5. 🔴 **Task Scheduler hiện chạy `Interactive` — cửa sổ `cmd` là một nút giết nhầm.** Cả 7 task (`dlck-ingester`, `dlck-ingester-measure`, `dlck-refdata`, 4 task OMO) đăng ký với `LogonType=Interactive`, `RunLevel=Limited`, user đang đăng nhập *(kiểm 2026-08-28)*. Ba hệ quả: (a) task **chỉ chạy khi có người đăng nhập** — log off là dừng cả ingester; (b) mỗi lượt chạy **hiện một cửa sổ `cmd`**, bấm nhầm dấu X trên cửa sổ của `dlck-ingester` là **giết phiên ghi tick**, thứ duy nhất không crawl lại được; (c) không có quyền admin — với các job hiện tại thì đủ. Chữa bằng `-LogonType S4U` *(cần admin)*: chạy cả khi không đăng nhập, không hiện cửa sổ. ⚠️ `scripts/register-tasks.ps1` **chưa nhận tham số đó** — nó gọi `Register-ScheduledTask` không kèm `-Principal` nên mặc định ra Interactive; sửa script là phần việc bắt buộc, không phải chạy lại là xong. ⚠️ **Đăng ký lại phải làm ngoài giờ chạy** — `Register-ScheduledTask -Force` lên task đang `Running` sẽ giết tiến trình; cửa sổ an toàn là sau **15:10** *(ingester ghi tới 15:05, bản đo tới 15:10)*. Việc này nằm ở [roadmap §5](../00-overview/roadmap.md).

...
5. ✅ **Lát 12 (2026-09-08): cơ chế Task Scheduler về hưu trong code.** `register-tasks.ps1` và `core/console.py` đã xoá; mọi tiến trình chạy bằng `docker compose` ở gốc repo, `ingester` là daemon tự ngủ ngoài phiên; `docker stop` gửi `SIGTERM`: job `etl` đóng sổ `ops.etl_run` và thoát 130, ingester đặt `stop` và đóng phiên đúng đường deadline (xả + đối chứng), thoát 0/1. 11 task Windows về hưu — gỡ bằng một lệnh PowerShell ở Task 12 (chủ dự án). Sau reboot, `restart: unless-stopped` đưa `api`/`etl`/`ingester` lên nhưng `depends_on` KHÔNG được đánh giá lại: ingester có thể trả 3 vài lượt tới khi kho healthy rồi tự ổn — ồn log, không mất dữ liệu (review lát 12). Phần dưới giữ làm lịch sử.
...
**Trong container** (lát 12, 2026-09-08): `/app/backend` (WORKDIR) · `/app/database` · `/var/lib/dlck/{logs,measure,spill}` (3 volume) · `/backups` (bind `CLICKHOUSE_BACKUP_DIR`) — **không có `/app/docs`**, code không đọc `docs/`, dữ liệu tra cứu ở `backend/etl/data/` (Task 10a). Compose: `docker-compose.yml` gốc + `docker-compose.vps.yml` qua `COMPOSE_FILE`.

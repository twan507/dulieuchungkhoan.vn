...
5. ✅ **Lát 12 (2026-09-08): cơ chế Task Scheduler về hưu trong code.** `register-tasks.ps1` và `core/console.py` đã xoá; mọi tiến trình chạy bằng `docker compose` ở gốc repo, `ingester` là daemon tự ngủ ngoài phiên, `SIGTERM` đi cùng đường Ctrl+C. 11 task Windows về hưu — gỡ bằng một lệnh PowerShell ở Task 12 (chủ dự án). Phần dưới giữ làm lịch sử.

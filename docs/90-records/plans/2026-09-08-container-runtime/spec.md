...
**§5.4 bước 4 "kiểm = 161" — code kiểm `> 0`** *(phát hiện ở review toàn nhánh 2026-09-08 tối, Spec M1)*. Spec viết "rồi kiểm `= 161`, thiếu ⇒ thoát 2"; `core.bootstrap.reseed_industry_if_needed` chỉ ném khi kết quả `= 0` — quyết định của code **đúng hơn spec**, vì chính seam §6 đòi ca "1 ticker ⇒ 1 dòng" (hardcode 161 sẽ vỡ ngay ca đó và vỡ tiếp khi bảng ánh xạ lớn thêm). Phép kiểm `= 161` vẫn sống, nhưng chỉ còn ở bước 4 thủ công của `database/README.md` (nơi sở hữu con số 161) — không phải trong `core.bootstrap`.

**§8 phép kiểm grep — mở rộng định nghĩa "vùng lịch sử"** *(Ruling #15, review toàn nhánh 2026-09-08 tối, Spec I2)*. Tiêu chí gốc chỉ nói "mọi hit còn lại phải thuộc `90-records/` hoặc `decisions/`" — bỏ sót hai hình dạng lịch sử khác cũng hợp lệ trong `roadmap.md`: mục **"Điểm vào cho lát N" đã gạch ngang cả tiêu đề** (`~~...~~`, ví dụ "Điểm vào cho lát 12") và **mục gạch ngang đã xong** trong bảng phán quyết/khảo sát (ví dụ dòng `~~Đăng ký lại 7 task…~~ ✅`). Cả hai đọc được là lịch sử ngay tại chỗ (gạch ngang + nhãn ✅/ĐÃ DÙNG XONG kèm ngày), nên không cần dời khỏi `roadmap.md` mới "sạch". Ba dòng còn nói thì-hiện-tại về file đã xoá bên trong các khối đó (`register-tasks.ps1` ở `roadmap.md:76`/`:753`, `stack.mjs` ở `:580`) đã thêm cụm `(đã xoá ở lát 12)` ngay sau tên file trong đợt sửa gộp sau review (2026-09-08). Dòng có ngày trong `reference-repos.md` cũng thuộc vùng lịch sử theo cùng lý lẽ (mốc đo, không phải hiện trạng).

**§6 seam hợp đồng env — vế "có người đọc" nay kiểm thật** *(Ruling #16, review toàn nhánh 2026-09-08 tối, Spec I1)*. Từ Task 2 tới tối 2026-09-08, `test_env_contract.py` chỉ kiểm "mọi khoá `.env.example` thuộc `KNOWN_KEYS`" — một danh sách viết tay CÙNG FILE với khoá đó (`core/env.py`), nên một khoá sống lâu hơn người đọc cuối cùng của nó (xoá consumer, giữ tên trong `.env.example` + `OPTIONAL_KEYS`) không bị bắt. Đợt sửa gộp sau review thêm hàm thuần `unread_keys(example_keys, readers)` và lắp `readers` = {tên `_READ` bắt trong code sản phẩm ngoài test} ∪ `REQUIRED_KEYS` (nguyên tố bảy `URL_SPECS` mà `compose_urls` đọc, cộng hai biến `CLICKHOUSE_API_USER`/`PASSWORD` chỉ `bootstrap` dùng gián tiếp qua tuple `CH_USERS` — không nằm trong `URL_SPECS` vì không có `CLICKHOUSE_API_URL`) ∪ khoá compose bơm qua `${TÊN...}` (`docker-compose.yml` + `docker-compose.vps.yml`) ∪ khoá bắt đầu `COMPOSE_`. Đo trên cây thật 2026-09-08: 0 khoá mồ côi.

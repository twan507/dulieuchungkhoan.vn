...
- **T3 `refdata_merge`**: xong (sonnet), commit `4438073`, 36 test cụm etl xanh. **Agent dừng đúng chỗ:** literal `stocks_no_issuer == 2` trong README fixture SAI — bản đầu đếm cả `L40_WFT_01`, dòng rác mà chính normalize lọc; agent giải tay từ fixture đã commit thay vì tin README, không tự sửa literal cho khớp code, không commit test đỏ. Controller kiểm lại bằng output đợt chụp gốc → agent đúng; sửa README + plan (`37efe07`) rồi mới cho agent sửa test theo. Đúng họ trôi-lệch §1.2, lần này ở meta file của chính mình.
- **Lỗi vệ sinh của controller (ghi để không lặp):** commit `37efe07` dùng `git add -A` khi agent T3 còn hai file chưa commit trong cây ⇒ code merge bị quét nhầm vào commit docs. Dữ liệu và test không ảnh hưởng; không viết lại lịch sử. Luật rút ra: controller commit khi có agent đang làm việc thì **luôn add theo pathspec tường minh**, không `-A`.
- **T5 `refdata_store`**: xong (sonnet), commit `566214c`, 42 test — controller kiểm tay: không DELETE/TRUNCATE, `apply` không tự commit, `industry_id` vắng trong mọi UPDATE, khớp security theo ticker một mình.
- **T6 `refdata_job` + CLI**: xong (sonnet), commit `9f691d7`, 45 test cụm etl · **217 toàn backend**.
- **T7 (controller)**: chạy thật dưới credential production TRƯỚC khi đăng ký task (§3.5). Lượt 1: exit 0, `sec_inserted=2015` (= 1.962 CP + 31 ETF + 18 chỉ số + 4 huỷ niêm yết — khớp TỪNG con số dự đoán từ đợt chụp), `stocks_no_issuer=437`, `junk_stocktype2=14`, `index_junk=2`. Lượt 2: `sec_inserted=0, sec_unchanged=2015` — idempotent, không `updated_at` nào bị đụng. Kho: security 2.015 · issuer 1.550 · sec_ext 2.014 (=1.993+18 snapshot+3 tvc, khớp tính tay) · iss_ext 1.550 · icb 176 · domain_state 2 dòng. Task `dlck-refdata` 08:00 đăng ký, `Assert-TaskCommand` soi lệnh thật, 7/7 task đúng lệnh. Quét tài liệu sống theo spec §10: roadmap (cây [3] + ghi chú nút thắt), architecture §3.1, market-data-store §4, database/README, 90-records/README — đã sửa cả 5, `git grep` đối chiếu.

...
- Từ điển nạp từ **hai nguồn**: 83 chỉ tiêu `GetScreenerParameters` (kèm `valueRange`) + 729 mã [field-dictionary.json](../../../../backend/etl/data/field-dictionary.json) (kèm `don_vi_du_lieu`, 392 mã đã xác thực bằng đẳng thức kế toán — bảy phép kiểm đó vào bộ giám sát hợp đồng, bước 7). **Luật ưu tiên khi trùng `code`** *(review 2026-08-25)*: `unit` lấy theo `field_dictionary` (phủ 99,7%, đã xác thực); `screener_params` chỉ là nguồn của `valueRange` và tên hiển thị.

...
**Tỷ giá — nay là các asset class `'fx'`:** sáu cặp ECB = sáu asset `fx.usd_eur` · `fx.usd_jpy` · `fx.usd_gbp` · `fx.usd_cad` · `fx.usd_sek` · `fx.usd_chf` (fixing 14:15 CET, `price_type='fixing'`); `fx.usd_cny` từ FRED `DEXCHUS` (noon ET); **5 chuỗi USD/VND** từ WiChart `dhtg` = `fx.usd_vnd.central/.ceiling/.floor/.bank_sell/.free_sell` (chuyển từ macro theo luật phân miền — bước 4 §0, vòng 3 I-4). Giá trị = **số quote trên 1 USD**, chiều tường minh ghi ở `unit` ('EUR/1 USD'); Frankfurter trả `EUR: 0.86453` = 1 USD đổi 0,86453 EUR — chiều hiển thị EURUSD = 1/giá trị, tính ở tầng đọc. Thêm nguồn khác mốc chốt (Yahoo close 23:00 UTC) = **asset mới** (`fx.usd_eur.close`), không đè chuỗi fixing.

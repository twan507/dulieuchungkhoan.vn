...
Kết quả sau sửa (task `dlck-price-backfill` chạy thật): cửa sổ thuộc `cmd.exe`, log có `[dlck] nút X của cửa sổ đã khoá — dừng bằng Ctrl+C hoặc Stop-ScheduledTask`, và **`GetMenuState(SC_CLOSE) = 4294967295`** (mục Close đã bị xoá). Trọn bộ test: **450 passed, 2 skipped**. Chủ dự án xác nhận *"cửa sổ OK"*, test Ctrl+C (trên lượt 85 chạy code cũ) ⇒ traceback và cửa sổ đóng ngay.

**Ba chỉnh sửa theo phản hồi cửa sổ thật** *(cùng ngày)*: (1) Ctrl+C ghi sổ `failed: dừng tay (Ctrl+C)`, thoát 130, con trỏ giữ (`b4f522f`); (2) **lời chào cuối** — output của job đã bị `>> log 2>&1` nuốt nên cửa sổ vốn không hiện gì, python ghi thẳng `CONOUT$` (kiểm thật qua wrapper: `say True`) một dòng ĐÃ DỪNG / THẤT BẠI / GUARD TỪ CHỐI / XONG rồi giữ cửa sổ 20 giây; (3) cửa sổ chỉ còn **tên task + một dòng mô tả** khai trong script (`-Description`, `chcp 65001` cho tiếng Việt), bỏ dòng dài có đường dẫn; job giá in thêm dòng `bắt đầu …` (giờ, con trỏ, còn bao nhiêu mã, hạn). Cả ba chỉ hoạt động khi wrapper đặt `DLCK_LOCK_CONSOLE=1` — chạy tay từ terminal không đợi, không khoá.

**Vòng phản hồi thứ hai trên cửa sổ thật** (ảnh chụp của chủ dự án): mô tả dài bị xuống dòng, "Ctrl+C dừng, con trỏ giữ nguyên" in hai lần, và **Ctrl+C không dừng — job "treo"**. Sổ + tiến trình cho thấy job vẫn chạy (4 tiến trình) sau Ctrl+C: nguyên nhân là **QuickEdit** của console cổ điển — bấm chuột vào cửa sổ là vào chế độ Select, tiến trình bị tạm dừng và **Ctrl+C thành copy** (tiêu đề `Select dlck-…` đã thấy ở lần đo trước). Sửa: (a) `disable_quick_edit()` bằng `SetConsoleMode` cùng cổng với khoá nút X (kiểm qua wrapper thật: mode `0x40` mất); (b) bỏ hẳn lời chào cuối 20 giây theo yêu cầu — dừng/xong là đóng ngay; (c) mô tả ≤ 80 ký tự, dòng `bắt đầu` chỉ còn giờ · con trỏ · còn N mã · hạn, không lặp. ⚠️ Console cổ điển có QuickEdit: **bấm chuột vào trong cửa sổ** là vào chế độ Select (tiêu đề hiện `Select …`) và **tiến trình bị treo** cho tới khi bấm Esc — đừng bấm vào trong cửa sổ, chỉ nhìn.

...
| [`2026-08-26-ingester-omo-first-slice/`](plans/2026-08-26-ingester-omo-first-slice/) | **Lát cắt dọc đầu tiên** — `ingester` realtime (socket BVSC → Redis → ClickHouse) + job `etl omo` (crawl OMO của SBV → Postgres) — `spec.md` · `plan.md` · `ledger.md` | 🟡 code xong + 2 vòng review (148 test xanh, OMO chạy thật); **chờ gate phiên đo trong giờ giao dịch** trước khi bật ghi thật |

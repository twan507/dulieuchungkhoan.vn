...
│  → kho-du-lieu-thi-truong.md               │  → pipeline-tin-tuc.md│
└───────┬─────────────────────────────────────────────┬──────────────┘
        │                                             │
┌───────▼─────────────────────────────────────────────▼──────────────┐
│  L2 · KHO                                                           │
...
│  → tang-ngu-nghia-chatbot.md                                        │
└───────┬─────────────────────────────────────────────────────────────┘
        │
┌───────▼─────────────────────────────────────────────────────────────┐
│  L4 · TRI THỨC — .claude/skills/                                     │
│  co-van-chung-khoan-vn   (tư duy, văn phong — luôn có mặt)          │
│  kien-thuc-chung-khoan-vn (công thức, quy trình — tải khi cần)      │
└───────┬─────────────────────────────────────────────────────────────┘
        │
...
|---|---|---|
| [`10-nguon-du-lieu/`](../10-nguon-du-lieu/) | Sự thật đo được về hệ thống của người khác | **Chỉ khi đo lại.** Sửa mà không đo là nói dối |
| [`20-thiet-ke/`](../20-thiet-ke/) | Lựa chọn của Finext | Được, nhưng ghi lý do vào [`quyet-dinh/`](quyet-dinh/) |
| [`30-tri-thuc/`](../30-tri-thuc/) | Nguyên liệu và nhật ký dựng skill | Corpus bất biến; ghi chú chỉ thêm, không xoá |
...
[Pipeline tin §8](../20-thiet-ke/pipeline-tin-tuc.md) cần *"danh sách ~1.600 mã HOSE/HNX/UPCoM"* cho tầng 2, và *"bảng tên thương mại → mã"* cho tầng 3. Cả hai đang nằm ở mục **Còn để ngỏ**.

Đáp án đã có trong [`getListOrganization`](../10-nguon-du-lieu/thi-truong/03-fiin-tham-chieu.md):
...
Hệ quả vận hành: bảng `organization` của [kho dữ liệu thị trường §5.1](../20-thiet-ke/kho-du-lieu-thi-truong.md) trở thành **phụ thuộc cứng của pipeline tin**. Pipeline tin không được tự nạp danh sách riêng — hai bản sao sẽ lệch nhau.
...
Việc xếp một ngành cụ thể vào bậc nào là **kết quả chạy lúc trả lời**, không phải hằng số trong tài liệu: lấy ngành từ ICB, chấm bốn thành phần rủi ro theo phương pháp ở [`danh-muc-va-luan-chuyen.md`](../../.claude/skills/kien-thuc-chung-khoan-vn/references/danh-muc-va-luan-chuyen.md), chia dải làm ba.

Nhờ vậy khung ngành đổi thì câu trả lời đổi theo, không phải sửa skill.

### 3.3 Function calling ↔ tầng skill

[Kho dữ liệu §6.3](../20-thiet-ke/kho-du-lieu-thi-truong.md) định nghĩa 5 function cho chatbot. [Skill L2](../../.claude/skills/kien-thuc-chung-khoan-vn/SKILL.md) viết: *"có công cụ tra dữ liệu thì gọi và dùng số hiện hành, không có thì nói rõ là cần tra"*.

Hai câu này nói về **cùng một thứ, từ hai phía** — và chưa bên nào biết bên kia tồn tại. Chi tiết hợp đồng ở [tầng ngữ nghĩa cho chatbot](../20-thiet-ke/tang-ngu-nghia-chatbot.md).
...
Đầy đủ: [ADR 0002](quyet-dinh/0002-chon-nguon-du-lieu.md).
...
| 10 báo điện tử | ⚠️ Có điều kiện | Xem mục bản quyền ở [pipeline tin §9.7](../20-thiet-ke/pipeline-tin-tuc.md) |

Hệ quả thiết kế: **ETL WiChart phải tách được khỏi hệ thống mà không kéo đổ thứ gì khác.** Nếu giấy phép không chốt được, mất 87 endpoint vĩ mô/hàng hoá nhưng phần cổ phiếu vẫn chạy. Nguyên tắc *ETL độc lập theo miền* ở [kho dữ liệu §9.4](../20-thiet-ke/kho-du-lieu-thi-truong.md) đã lo việc này — giữ nó, đừng gộp cho gọn.

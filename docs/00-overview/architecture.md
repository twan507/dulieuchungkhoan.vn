...
Hệ quả vận hành *(cập nhật 2026-08-26 đêm: danh bạ ĐÃ NẠP THẬT — job `etl refdata` hằng ngày 08:00, 2.015 mã, 1.550 doanh nghiệp; **ngành: nội dung thiết kế xong 2026-08-27 nhưng CHƯA NẠP** — `industry_id` vẫn rỗng, chờ migration seed, nên tầng lọc ngành của tin vẫn chặn)*: bảng danh bạ — nay là **`market.issuer` + `market.security`** (spec schema 2026-08-25 tách đôi khái niệm `organization` cũ: doanh nghiệp phát hành vs mã giao dịch — xem [spec bước 2](../90-records/plans/2026-08-25-postgres-data-schema/step-02-market-identity.md)) — trở thành **phụ thuộc cứng của pipeline tin**. Pipeline tin không được tự nạp danh sách riêng — hai bản sao sẽ lệch nhau. Gắn mã chỉ nhận `security` có `status='listed'`.

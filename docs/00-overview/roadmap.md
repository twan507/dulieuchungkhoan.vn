...
**Nợ để lại (nhỏ, có lý do):** hai `conftest` tạo hai fixture `migrated_engine` session-scope ⇒ DB test migrate hai lần mỗi lượt full suite (dọn một lượt riêng, cả họ) · `open_run` ngoài `try/finally` ở cả 7 job (ledger lát 4) · phép kiểm tên series bất đối xứng (tên API rất ngắn khớp mọi series "Giá …") · `Series.calendar` vô nghĩa cho macro · giờ nạp của WiChart chưa đo (giả định trước 08:00 giờ VN, ảnh hưởng lát 13).

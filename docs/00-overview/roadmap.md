...
**Trạng thái bàn giao 2026-09-04 ~23:00:** `main` = lát 5 + fix mốc nước lát 4 + roadmap 14 lát · **593 test xanh, 2 skipped** (`pytest tests -q`) · migration head `0017` · `financial_statement` **27,3 triệu dòng / 1.523 mã / 3,4 GB**, `financial_report_file` 114.629 dòng, mốc nước `2026-09-04` · `metric_dictionary` 729 dòng · **không đăng ký task Scheduler** (lịch thuộc lát 13: `fundamentals` chạy **sau `events` 18:10 và sau `snapshot`**).
...
| 🔴 **Nợ code còn sống, đã biết từ lát 3 mà chưa vá:** `screener_fetch.py:40` và `events_fetch.py:47` kiểm `status == "Success"`, thiếu nhánh `0` (quy ước §6.1). Bộ giám sát áp đúng công thức sẽ "phát hiện" lại nó — đó là nợ cũ, không phải lỗi mới. Việc đầu tiên của lát 6: vá hai dòng này (TDD, mẫu `status: 0` có sẵn ở fixture snapshot) | [quy ước §6.1](../10-sources/market/00-conventions.md) · [bài học lát 3 #2](#lát-3--giá-theo-ngày--xong-2026-09-04) |

...
lát 6   vĩ mô WiChart            ✅ XONG 2026-09-05 — 68 key (67 §9 + ca_tra sống lại; 20 Tier X) → macro.observation (53 series)
                                   ...
                                   37.629 điểm/lượt, 15 giây; bằng chứng thô khi hash đổi; series_break GDP 2026-01-01 ×1,6005. TIẾP: lát 7
lát 7   quốc tế                  FRED · Frankfurter · Yahoo · LBMA · Binance → macro.observation + asset.price_daily/ohlc_daily
...
**Nợ để lại (nhỏ, có lý do):** ~~hai `conftest` tạo hai fixture `migrated_engine`~~ ✅ **đã gộp về một `backend/tests/conftest.py` cùng ngày** (DB test dựng một lần; hai test schema lộ va chạm với dòng test job đã commit — `VNINDEX`, 729 mã từ điển — đổi sang literal `ZZ*`, 640 test xanh) · `open_run` ngoài `try/finally` ở cả 7 job (ledger lát 4) · phép kiểm tên series bất đối xứng (tên API rất ngắn khớp mọi series "Giá …") · `Series.calendar` vô nghĩa cho macro · giờ nạp của WiChart chưa đo (giả định trước 08:00 giờ VN, ảnh hưởng lát 13).

### Điểm vào cho lát 7 — quốc tế (FRED · Frankfurter · Yahoo · LBMA · Binance), đọc trước khi bắt đầu

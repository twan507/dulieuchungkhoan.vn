...
**Trạng thái bàn giao 2026-09-04:** nhánh `feat/price-daily-etl` merge `main` · **438 test xanh** · migration head `0015` · 9 task `Disabled` + `dlck-price` chờ đăng ký (AC8) · `price_daily` 113.427 dòng: 60 phiên/mã cho cả 1.523 mã + backfill đã đi tới con trỏ `ABC` (kiểm `ops.etl_run` job `market.price_backfill`, `stats.cursor`).
...
| Backfill lịch sử giá **còn dở** (10/1.523 mã + BID): `--backfill --max-minutes N` mỗi đêm ngoài giờ giao dịch, N hết trước giờ ngủ 02:00 (máy ngủ giữa chừng cũng không hỏng — job sống qua giấc ngủ, chỉ dừng sớm), tới khi `stats.pass_complete = true` (~20 giờ tuần tự); đừng chạy song song với lượt 15:40 | [backend/README](../../backend/README.md) |

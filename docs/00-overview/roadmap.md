...
| **Code sản phẩm (ingester · ETL thật · api)** | 🟡 **Lát cắt dọc đầu đã dựng 2026-08-26** — `ingester` (socket EIO3 → chuẩn hoá → Redis + ClickHouse, leader lock, đối chứng cuối phiên) và job `etl omo`; **194 test xanh** trên Postgres/Redis/ClickHouse thật, đã qua review toàn nhánh và **merge `main` 2026-08-26**. Job OMO đã chạy thật từ 26/08 (4 mốc/ngày) — nhưng ⏸️ **cả 4 task OMO đã `Disabled` lúc 2026-08-28 15:04** *(quyết định chủ dự án: giai đoạn này ưu tiên dev, đã test đủ)*. ⚠️ **Đồng hồ mất dữ liệu OMO vì thế chạy lại**: SBV không cho backfill, mỗi ngày tắt là mất vĩnh viễn phần ngày đó *(`sbv-omo.md` Giới hạn 1)*. Bật lại: `Get-ScheduledTask -TaskName "dlck-omo-*" | Enable-ScheduledTask`. **Ghi tick bật 2026-08-26 tối** — phiên ghi thật đầu tiên là 27/08, chạy song song một phiên `--measure` làm lưới an toàn. `api` chưa bắt đầu | [plans/2026-08-26-ingester-omo-first-slice/](../90-records/plans/2026-08-26-ingester-omo-first-slice/) |
...
| **4c** | **Lát tràn-ra-đĩa** ([spec](../90-records/plans/2026-08-28-ingester-spill-to-disk/spec.md)) — đóng chỗ hở §2.1 dưới (`pending` không trần). 🟡 **Code xong 2026-08-27 tối** — hai vòng quản/ghi tách biệt, hai cửa vào chế độ đĩa, hai loại file `-r`/`-n`, bộ đếm `d[]` (`--count`); N/K/trần đĩa điền theo số đo gate cùng ngày (spec §2.5). **AC2 (chaos, `docker stop` thật) PASS có số**: 200.000 fed / 200.000 stored / 0 dup / RSS đỉnh 78,7 MB / 253 block spill+replay. **AC3 (hằng đẳng thức sổ sách một phiên thật) chờ phiên 28/08** — hồ sơ: [ledger](../90-records/plans/2026-08-28-ingester-spill-to-disk/ledger.md) | Đóng đúng chỗ hở mà [4b] chỉ che tạm bằng lưới an toàn — không có nó thì ATO mạnh trùng lúc CH trục trặc vẫn OOM mất dữ liệu im lặng |
...
| 🔴 **Đăng ký lại 7 task với `-LogonType S4U`** | Hiện cả 7 task chạy `Interactive` *(kiểm 2026-08-28)*: chỉ chạy khi có người đăng nhập, và mỗi lượt hiện một cửa sổ `cmd` — bấm nhầm dấu X trên cửa sổ `dlck-ingester` là **giết phiên ghi tick**. Chi tiết và ba hệ quả: [service-topology §5](../20-design/service-topology.md) | 🔴 **Không chỉ chạy lại script** — `scripts/register-tasks.ps1` **chưa có tham số `-LogonType`**, nó gọi `Register-ScheduledTask` không kèm `-Principal` nên luôn ra Interactive; phải **sửa script trước**. Rồi chạy bằng quyền admin, **sau 15:10** — `-Force` lên task đang `Running` sẽ giết tiến trình. Nghiệm thu: `Get-ScheduledTask -TaskName "dlck-*"` báo đủ 7 dòng `LogonType=S4U`, và lượt chạy kế tiếp không hiện cửa sổ. Các bước: [chuỗi việc sau phiên 28/08 — Task 2](../90-records/plans/2026-08-28-post-session-sequence/plan.md) |
| 🔴 **Luật huỷ niêm yết cho mã vắng danh bạ** | Job `etl refdata` chỉ lật `delisted` cho chiều *có trong danh bạ mà vắng bảng giá*; chiều ngược lại — **có trong bảng giá BVSC mà vắng khỏi danh bạ FiinTrade** — chưa có luật nào. Đo 2026-08-28: **438 cổ phiếu** không có issuer vẫn mang nhãn `listed` (UPCOM 378 · HNX 39 · HOSE 21), gồm cả Habubank đã sáp nhập SHB từ 2012. Ảnh hưởng mọi thống kê *"mã đang niêm yết"* và làm ETL giá gọi API cho 438 mã chết mỗi ngày | Chốt **lưới chống bắn nhầm mã mới niêm yết** (vắng bao nhiêu lượt liên tiếp thì mới lật) rồi làm **cùng lát phái sinh + `/datafeed/instruments`** — ba việc sửa cùng `refdata_merge`/`plan_delist`/`refdata_guard`. Phải xong **trước [7] ETL giá**. Chi tiết luật và ba ràng buộc: [market-data-store §4.4](../20-design/market-data-store.md); **kế hoạch thực thi đã viết 2026-08-28** (cột dấu `security.directory_absent_since`, ngưỡng 3 ngày, 3 task): [plans/2026-08-28-catalog-delisting-rule/](../90-records/plans/2026-08-28-catalog-delisting-rule/); xếp trong [chuỗi việc sau phiên 28/08](../90-records/plans/2026-08-28-post-session-sequence/plan.md) |

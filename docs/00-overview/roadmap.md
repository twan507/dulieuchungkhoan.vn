...
**Trạng thái bàn giao 2026-09-04:** nhánh `feat/price-daily-etl` merge `main` · **444 test xanh** · migration head `0015` · **11 task** đã đăng ký thật, đều `Disabled` · `price_daily` 113.427 dòng: 60 phiên/mã cho cả 1.523 mã + backfill đã đi tới con trỏ `ABC` (kiểm `ops.etl_run` job `market.price_backfill`, `stats.cursor`).
...
| Backfill lịch sử giá **còn dở** (con trỏ ở `ACC`, 17/1.523 mã + BID + DMX xong): chạy bằng task **`dlck-price-backfill`** (thứ 7 00:05, hoặc `Start-ScheduledTask` tay buổi tối; `--stop-before-open` tự dừng trước 08:45 ngày giao dịch kế; ~20 giờ đủ trọn vòng) — task **đã đăng ký thật** 2026-09-04 (AC8), đang `Disabled` cùng cả đội: `Enable-ScheduledTask dlck-price-backfill; Start-ScheduledTask dlck-price-backfill` là chạy; máy ngủ giữa chừng cũng không hỏng | [backend/README](../../backend/README.md) |

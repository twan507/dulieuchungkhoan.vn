...
| 🔴 **Đăng ký lại 7 task với `-LogonType S4U`** | Hiện cả 7 task chạy `Interactive` *(kiểm 2026-08-28)*: chỉ chạy khi có người đăng nhập, và mỗi lượt hiện một cửa sổ `cmd` — bấm nhầm dấu X trên cửa sổ `dlck-ingester` là **giết phiên ghi tick**. Chi tiết và ba hệ quả: [service-topology §5](../20-design/service-topology.md) | Chạy `scripts/register-tasks.ps1` bằng quyền admin với `-LogonType S4U`, **sau 15:10** — `-Force` lên task đang `Running` sẽ giết tiến trình. Nghiệm thu: `Get-ScheduledTask -TaskName "dlck-*"` báo `LogonType=S4U`, và lượt chạy kế tiếp không hiện cửa sổ |

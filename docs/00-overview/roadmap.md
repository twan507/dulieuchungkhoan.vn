...
### Điểm vào cho lát 3 — giá theo ngày, đọc trước khi bắt đầu

**Trạng thái bàn giao 2026-09-03:** `main` sạch sau merge lát 2 · **394 test xanh** · 9 task Scheduler đều `Disabled` (xem [4d]) · migration head `0015`.

**Lát 3 = giá theo ngày** — `getPriceData`, **1.974 lời gọi/ngày** (Page 1 của mỗi mã, sau 15:00) · backfill toàn bộ 52 trang × 1.974 mã = **102.648 lời gọi** (~12 giờ ở 8 luồng, giới hạn 2 request/giây, rải 1–2 tuần — [`market-data-store.md` §4.2](../20-design/market-data-store.md)). Đây là lát đầu tiên thật sự cần lớp `core/http` dùng chung + token bucket: các lát trước (screener 52 trang, events 9 lời gọi) đều gọn trong một lượt tuần tự.

| Cần biết trước | Ở đâu |
|---|---|
| Tài liệu endpoint `getPriceData` — 97 trường/phiên, hai bẫy tham số (`Code` phải là `organCode`, không phải ticker), độ sâu phân trang lùi tới 2014, đây là nguồn CHUẨN cho `openInterest` phái sinh | [`09-fiin-market-price.md`](../10-sources/market/09-fiin-market-price.md) |
| **Nhịp 8 luồng chưa ai đo** — kết luận đứng nguyên sau khi ngân sách ngày giảm xuống ≈ 2.300 lời gọi (họ Snapshot chuyển sang kích hoạt theo sự kiện); phải đo lại **đúng nhịp 8 luồng** trước khi bật chạy thật | [`00-conventions.md` §10.6](../10-sources/market/00-conventions.md) |
| Rate limiter dùng chung ngân sách giữa ETL và chatbot, token bucket riêng theo host | [`market-data-store.md` §4.3](../20-design/market-data-store.md) |
| Tín hiệu kích hoạt re-crawl giá theo sự kiện quyền: `market.corporate_event WHERE exright_date >= current_date` — AC5 của lát 2 đã xác nhận > 0 dòng trên kho thật | [ledger lát 2](../90-records/plans/2026-09-03-events-daily-etl/ledger.md) |
| Khuôn job để nhân bản: fetch → normalize → merge → **guard trước commit** → apply → `close_run` | `backend/etl/events_*.py` — lát 2, mới nhất, đã qua review toàn nhánh |

**Bốn bài học của lát 2, áp thẳng được:**

1. **Tham số lọc theo ngày (`FromDate`/`ToDate`) không tự nhiên đồng nhất giữa các endpoint cùng nhóm — đo trước khi tin, đừng suy từ tên tham số.** Lát 2 định dùng `FromDate` để chỉ lấy phần mới; đo lại 2026-09-03 lộ ra mỗi họ Calendar lọc theo một trục ngày khác nhau, và `Earning` lọc theo trường **không có trong response** — cửa sổ 5 ngày trả 24 bản ghi trong khi có 217 bản ghi thật nằm trong đúng cửa sổ đó. Tải trọn hoá ra rẻ hơn tưởng (9 lời gọi, ~140 giây cho cả sáu họ), nên cuối cùng không dùng `FromDate` nữa. `getPriceData` cũng có `FromDate`/`ToDate` riêng — kiểm bằng đúng phép so **tập bản ghi**, không suy diễn.
2. **`PageSize` có thể không có trần thực tế — đo trước khi thiết kế vòng phân trang.** Nhóm Calendar thử tới `PageSize=20.000` vẫn đúng, nguồn trả `min(PageSize, số còn lại)`. `getPriceData` dùng lối phân trang khác (52 trang cố định, lùi tới 2014) — đừng mang giả định của nhóm Calendar sang mà không đo lại.
...
**Lát 2 đã đóng trọn** *(2026-09-03, [ledger](../90-records/plans/2026-09-03-events-daily-etl/ledger.md))*: 110.695 dòng vào `market.corporate_event`, 517 issuer tối thiểu tạo mới, 42 bản ghi gộp vì đụng khoá tự nhiên, lượt hai idempotent, 394 test xanh.

...
**Trạng thái bàn giao 2026-09-04:** nhánh `feat/price-daily-etl` merge `main` · **449 test xanh** · migration head `0015` · **11 task** đã đăng ký thật, đều `Disabled` · `price_daily` 113.427 dòng: 60 phiên/mã cho cả 1.523 mã + backfill đã đi tới con trỏ `ABC` (kiểm `ops.etl_run` job `market.price_backfill`, `stats.cursor`).

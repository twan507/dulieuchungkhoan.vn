...
Tài liệu này ghi **lựa chọn của Finext**: kiến trúc xử lý, taxonomy 20 sub, quy tắc phân loại, cách gắn mã cổ phiếu, lược đồ kho. Phần *nguồn tin có gì và cư xử thế nào* — 47 feed, 6 crawler, encoding, khối lượng đo được — nằm ở [danh mục nguồn tin](../10-nguon-du-lieu/tin-tuc/README.md).

> **Đánh số mục kế thừa tài liệu gốc v3** và cố ý không đánh lại, vì hàng chục tham chiếu chéo dạng *"xem mục 6.5"* nằm rải trong cả hai file. Mục 4, 5, 6, 11.1–11.3 và 13 nằm ở [danh mục nguồn tin](../10-nguon-du-lieu/tin-tuc/README.md); các mục còn lại ở file này.

**File máy đọc đi kèm:** [`config/feeds.json`](../../config/feeds.json).
...
> Mục 11.1–11.3 (nguồn, cách lấy tin và feed bị loại) nằm ở [danh mục nguồn tin](../10-nguon-du-lieu/tin-tuc/README.md) vì chúng là dữ kiện về nguồn. Ở đây chỉ giữ phần lựa chọn thiết kế.
...
| File | Nội dung |
|---|---|
| `docs/20-thiet-ke/pipeline-tin-tuc.md` | tài liệu này — lựa chọn thiết kế |
| `docs/10-nguon-du-lieu/tin-tuc/README.md` | danh mục nguồn và đặc tính kỹ thuật |
| `config/feeds.json` | 47 feed + taxonomy + nhật ký loại bỏ, dạng máy đọc |

> Tài liệu gốc một file `THIET_KE_PIPELINE.md` v3 đã được tách làm hai theo ranh giới *dữ kiện về nguồn* / *lựa chọn của Finext*; `feeds_config_v3.json` đổi tên thành `config/feeds.json`. Nội dung không đổi một chữ nào.
...
*Tài liệu này đi kèm [`config/feeds.json`](../../config/feeds.json) — danh sách 47 feed ở dạng máy đọc được.*

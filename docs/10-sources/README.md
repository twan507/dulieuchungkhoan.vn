...
| [`field-dictionary.json`](../../backend/etl/data/field-dictionary.json) | **Từ điển máy đọc 729 mã chỉ tiêu BCTC** — tên VI/EN 98,5%, đơn vị dữ liệu 99,7%, phủ 100% response thật của ba endpoint BCTC (file nằm trong code từ 2026-09-08) | — |
...
| [`verify_wichart.py`](macro/verify_wichart.py) | **Script tự kiểm chứng** — đọc registry từ `backend/etl/wichart_source.py` rồi đối chiếu 509 khẳng định với API sống. Dùng làm bộ giám sát hợp đồng hàng ngày | — |
...
| [`news/feeds.json`](../../backend/etl/data/feeds.json) | Cùng nội dung ở dạng máy đọc — feed, taxonomy 21 sub, nhật ký loại bỏ (file nằm trong code từ 2026-09-08) | 47 feed |
...
| [market-field-selection.md](../20-design/market-field-selection.md) | **Chọn trường cho ETL** — lấy/bỏ từng mã trường, nguồn chuẩn, lý do tại chỗ. 279 dòng *(213 ở bản đầu 2026-08-14; bảng nới hai lần 2026-08-15 và 2026-09-03 — số hiện hành do chính `market-field-selection.md` sở hữu)*, kèm [bản JSON máy đọc](../../backend/etl/data/market-field-selection.json), sinh tự động từ [`gen_field_selection.py`](../20-design/gen_field_selection.py) |
...
2. **Dùng bảng hệ số đơn vị đã hardcode** ở [`backend/etl/wichart_source.py`](../../backend/etl/wichart_source.py) (từng là wichart.md §9). Nhãn `unit` của API sai ở 15 series, sai lệch 1000 lần, **rải rác ngẫu nhiên không theo quy luật nào** — kể cả giữa hai series cùng họ sản phẩm.
...
| **4.1** | **2026-08-14** | **Giải mã 729 mã chỉ tiêu** — toàn bộ họ `bs*`, `is*`, `cf*`, `nob*` cho cả bốn loại hình doanh nghiệp. Nguồn là bundle JS của ứng dụng FiinTrade, **không phải API**. Độ phủ đo trên 21 response thật của 5 mã: **100%**. Xem [Phụ lục A §A.5](market/appendix-A-field-codes.md) và [field-dictionary.json](../../backend/etl/data/field-dictionary.json) |

...
| **Bỏ — đã quan sát** | `div.ads_middle` (11) và `div[id^=adsWeb_]` (12) — quảng cáo, rỗng text nhưng cắt câu · `figure.article__avatar` (**1/4 bài**, rỗng text) · `a.cms-relate` (**3 node, chỉ ở 1/4 bài** — link *"Chi tiết"* trong bài tổng hợp, 24 ký tự) · `p.imgdesc` — chú thích ảnh nằm ngoài `figure` *(đo 2026-09-06 trên kho đã nạp: 26/453 bài mở đầu bằng caption dạng này)* |
...
| **Bẫy riêng** | ① Bài dạng tổng hợp *"Thị trường tài chính 24h"* kết mỗi mục bằng `..>>` + `<a class="cms-relate">Chi tiết</a>`. Bỏ thẻ `a` rồi thì **chuỗi `..>>` vẫn còn** vì nó là text node đứng trước — cần thêm luật văn bản dọn `\.\.>>\s*`. ② Đây là dạng bài *nhiều tin trong một*, nội dung trộn vàng, ngoại tệ, dầu, bitcoin, chứng khoán Mỹ/Á — cắt trần 3–4 K ký tự sẽ cắt mất phần chứng khoán Việt nằm cuối. ③ Nguồn nhẹ nhất bộ (94–101 KB/trang) và **sạch nhất về nội dung**: *(đo 05/09/2026, cỡ mẫu 4 bài)* 3/4 bài không bị bỏ một ký tự nào — các selector có khớp node nhưng đều là khối quảng cáo rỗng text. Ngoại lệ đo lại 2026-09-06 trên kho đã nạp: `p.imgdesc` (chú thích ảnh ngoài `figure`) có khớp node thật ở 26/453 bài — đã thêm vào luật bỏ ở trên. ④ **Đo 2026-09-05, sitemap:** `meta.cms-date[itemprop=datePublished]` trên trang là giờ đăng thật; `lastmod` của sitemap tháng ([README §5.2](README.md)) là giờ **SỬA** bài, không phải giờ đăng — ETL lấy giờ trang, `lastmod` chỉ để dự phòng |
